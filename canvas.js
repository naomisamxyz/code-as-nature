(function () {
  ...
  function measurePipelineDiagram(element, resource) {
    ...
    requestAnimationFrame(() => {
      element.style.height = "auto";
      resource.height = Math.ceil(element.scrollHeight);
      element.style.height = "";
      element.style.setProperty("--h", resource.height + "px");
      drawConnections();
      saveLocal();
  ...
  function loadPipelineDiagram(element, resource) {
    element.classList.add("diagram-object", "pipeline-diagram-object");
    ...
    mount.textContent = "Loading computational pipeline…";
    element.append(heading, mount);

    fetch((resource.source || "assets/diagram/index.html") + "?embed=2")
      .then(response => {
        if (!response.ok) throw new Error("Diagram could not be loaded");
        return response.text();
      })
      .then(markup => {
        const parsed = new DOMParser().parseFromString(markup, "text/html");
        const frame = parsed.querySelector(".diagram-frame");
        if (!frame) throw new Error("Diagram markup is missing");
        ...
        mount.addEventListener("click", event => {
          ...
          const group = button.closest(".step-group");
          const wasOpen = group.classList.contains("active");
          frame.querySelectorAll(".step-group").forEach(item => {
            item.classList.remove("active");
            item.querySelector(".step")?.classList.remove("active");
          });
          if (!wasOpen) {
  ...
  function createNode(resource) {
    ...
    if (resource.type === "week") {
      ...
      loadPipelineDiagram(element, resource);
